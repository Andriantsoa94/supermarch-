...
class CongeModel extends Model
{
    ...
    public function congeGroupByType($user)
    {
        return $this->select('types_conge.libelle, SUM(conges.nb_jours) as total_jours, COUNT(conges.id) as total_demandes')
            ->join('types_conge', 'types_conge.id = conges.type_conge_id', 'INNER')
            ->where('employe_id', $user)
            ->groupBy('types_conge.libelle')
            ->findAll();
    }
